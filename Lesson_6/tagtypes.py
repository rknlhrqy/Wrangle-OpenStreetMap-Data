# -*- coding: utf-8 -*-
"""
Created on Thu Jan 14 22:05:16 2016
...
def key_type(element, keys):
    if element.tag == "tag":
        ...
        if m:
            keys['lower'] += 1
            return keys
        m = lower_colon.search(element.attrib['k'])        
        if m:
            keys['lower_colon'] += 1
            return keys
        m = problemchars.search(element.attrib['k'])        
        if m:
            keys['problemchars'] += 1
            return keys
        keys['other'] += 1        
...
def process_map(filename):
    ...
    for events, element in ET.iterparse(filename, events=('start',)):
